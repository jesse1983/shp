...
export default () => {
    ...
    const viewCustomer = async (c) => {
        const [customer, addresses] = await Promise.all([getCustomer(c.customerId), getAddresses(c.customerId)]);
    ...
    return <Container maxWidth={1} className={`list-component count-${state.countCustomers}`}>   
        ...
        <div className="list-options-bar">
            {state.countCustomers > 0 && <Box>
                <Trans i18nKey="total customers" count={state.countCustomers} />
            </Box>}
            {state.countCustomers > 12 && <Box className="list-options-bar-pagination">
                <Fab aria-label="next" onClick={() => dispatch({ type: 'PREV_PAGE' })}>
                    <NavigateBeforeIcon />
                </Fab>
                <Typography><Trans i18nKey="current to total" values={{ currentPage, totalPages }} /></Typography>
