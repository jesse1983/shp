...
export default () => {
    ...
    const validateProvisory = (prov) => CustomerSchema.validate(prov || provisory || {}, { abortEarly: false });
    ...
    return <Container maxWidth="lg" className={`view-customer ${mode}`}>
        ...
        {customer && <>
            ...
            <Box>
                {editing ? <>
                    <div className="grid-2-columns">
                        <TextField onInput={inputField} defaultValue={provisory.firstName} error={!!errorMessage('firstName')} id="firstName" variant="outlined" label={t('First name')} fullWidth></TextField>
                        <TextField onInput={inputField} defaultValue={provisory.lastName} error={!!errorMessage('lastName')} id="lastName" variant="outlined" label={t('Last name')} fullWidth></TextField>
                    </div>
                </> : <>
            ...
            <Box>
                {editing ? <>
                    <TextField onInput={inputField} defaultValue={provisory.additional} error={!!errorMessage('additional')} id="additional" variant="outlined" label={t('Additional')} fullWidth></TextField>
                </> : <>
                    <Typography>{customer.additional}</Typography>
                </>}
            </Box>
            {Array.isArray(customer.addresses) && customer.addresses.map((address, i) => <Box key={address}>
                {editing ? <>
                    {ADDRESS_FIELDS.map((e) => <Box key={e.field}>
                    ...
                    <Typography>{[address.street, address.streetNumber, address.additional].filter(e => !!e).join(', ')} </Typography>
                    <Typography>{[address.district, address.city, address.state, address.country].filter(e => !!e).join(', ')} </Typography>
                </>}
            </Box>)}
            <Box>
                {editing ? <>
